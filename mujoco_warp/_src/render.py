...
def tile_render_megakernel(m: Model, d: Data, rc: RenderContext):
  ...
  def _tile_occupancy_kernel(
    ...
    # AABB that bounds the near/far corners of the tile frustum.
    near_min_x = wp.min(wp.min(near_tl[0], near_tr[0]), wp.min(near_bl[0], near_br[0]))
    far_min_x = wp.min(wp.min(far_tl[0], far_tr[0]), wp.min(far_bl[0], far_br[0]))
    min_x = wp.min(near_min_x, far_min_x)

    near_min_y = wp.min(wp.min(near_tl[1], near_tr[1]), wp.min(near_bl[1], near_br[1]))
    far_min_y = wp.min(wp.min(far_tl[1], far_tr[1]), wp.min(far_bl[1], far_br[1]))
    min_y = wp.min(near_min_y, far_min_y)

    near_min_z = wp.min(wp.min(near_tl[2], near_tr[2]), wp.min(near_bl[2], near_br[2]))
    far_min_z = wp.min(wp.min(far_tl[2], far_tr[2]), wp.min(far_bl[2], far_br[2]))
    min_z = wp.min(near_min_z, far_min_z)

    near_max_x = wp.max(wp.max(near_tl[0], near_tr[0]), wp.max(near_bl[0], near_br[0]))
    far_max_x = wp.max(wp.max(far_tl[0], far_tr[0]), wp.max(far_bl[0], far_br[0]))
    max_x = wp.max(near_max_x, far_max_x)

    near_max_y = wp.max(wp.max(near_tl[1], near_tr[1]), wp.max(near_bl[1], near_br[1]))
    far_max_y = wp.max(wp.max(far_tl[1], far_tr[1]), wp.max(far_bl[1], far_br[1]))
    max_y = wp.max(near_max_y, far_max_y)

    near_max_z = wp.max(wp.max(near_tl[2], near_tr[2]), wp.max(near_bl[2], near_br[2]))
    far_max_z = wp.max(wp.max(far_tl[2], far_tr[2]), wp.max(far_bl[2], far_br[2]))
    max_z = wp.max(near_max_z, far_max_z)
  ...
  def _tile_render_megakernel(
    ...
    active = (ix < W) and (iy < H)

    if not active:
      return

    ray_offset = cam_ray_adr[cam_idx]
    ray_idx = ray_offset + iy * W + ix
    dir_local = ray[ray_idx]
    ...
    # Case: Overflow (> 4 Geoms) -> Full BVH Traversal
    if occ[0] == -3:
    ...
    # Case: Optimized Loop (1-4 Geoms)
    else:
        # Loop through the specific candidates found in occupancy
        # We unroll this loop because we know max is 4
        for i in range(4):
            candidate_id = occ[i]
            if candidate_id == -1:
                break
            
            # NOTE: You need to implement/expose a function that intersects 
            # a single primitive WITHOUT traversing the BVH.
            # Let's call it `intersect_primitive`.
            d, n, u, v, f, m_id = intersect_primitive(
                candidate_id, world_idx, 
                geom_type, geom_dataid, geom_size, geom_xpos, geom_xmat,
                mesh_faceadr, mesh_face, mesh_bvh_id, hfield_bvh_id,
                ray_origin_world, ray_dir_world
            )
    ...
    if render_depth[cam_idx]:
      depth_out[world_idx, depth_adr[cam_idx] + ray_idx] = hit_dist
    ...
    rgb_out[world_idx, rgb_adr[cam_idx] + ray_idx] = pack_rgba_to_uint32(
      hit_color[0] * 255.0,
      hit_color[1] * 255.0,
      hit_color[2] * 255.0,
      255.0,
